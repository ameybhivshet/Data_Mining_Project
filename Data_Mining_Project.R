...
data.model <- specifyModel(T.ind(GSPC) ~ Delt(Cl(GSPC), k = 1) +
                             myATR(GSPC) + myADX(GSPC)  + myVolat(GSPC) +
...
###svm model1###
sv1 <- svm(Tform, Tdata.train[1:7000, ], gamma = 0.001, cost = 100)
svm.preds1 <- predict(sv1, Tdata.train[7001:10316, ])
sigs.svm1 <- trading.signals(svm.preds1, 0.1, -0.1)
true.sigs.svm1 <- trading.signals(Tdata.train[7001:10316,"T.ind.GSPC"], 0.1,-0.1)
sigs.PR(sigs.svm1, true.sigs.svm1)     




#svm model2###
sv2 <- svm(Tform, Tdata.train[1:7000, ], gamma = 0.01, cost = 10)
svm.preds2 <- predict(sv2, Tdata.train[7001:10316, ])
sigs.svm2 <- trading.signals(svm.preds2, 0.1, -0.1)
true.sigs.svm2 <- trading.signals(Tdata.train[7001:10316,"T.ind.GSPC"], 0.1,-0.1)
sigs.PR(sigs.svm2, true.sigs.svm2)     





###prediction on validation set using model 1###


svm.preds.val <- predict(sv1, Tdata.eval)
sigs.svm.val <- trading.signals(svm.preds.val, 0.1, -0.1)
true.sigs.svm.val <- trading.signals(Tdata.eval[,"T.ind.GSPC"], 0.1,-0.1)
sigs.PR(sigs.svm.val, true.sigs.svm.val)     
...
# 
addpredT <- addTA(ta=svm.preds.val,col="blue", legegend = "Predicted T",)
addpredT
